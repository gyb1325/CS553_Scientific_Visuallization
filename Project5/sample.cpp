...
const char *WINDOWTITLE = { "Project 5 -- Yongbin Gu" };
	...
	for (int dx = 0; dx < NX; dx++) {
		for (int dy = 0; dy < NY; dy++) {
			for (int dz = 0; dz < NZ; dz++) {
				...
				float tail[3], head[3];
				tail[0] = x - Scale_Factor * vx / 2.;
				tail[1] = y - Scale_Factor * vy / 2.;
				tail[2] = z - Scale_Factor * vz / 2.;
				head[0] = x + Scale_Factor * vx / 2.;
				head[1] = y + Scale_Factor * vy / 2.;
				head[2] = z + Scale_Factor * vz / 2.;
