...
const char *WINDOWTITLE = { "Project04 --Yongbin Gu" };
...
const char *SFORMAT = { "KValue: %.3f - %.3f" };
...
void    ConvertHyperbolic(float *, float * );
void    ConvertPolar(float *, float *);
void    ConvertCartisan(float *, float *);
void    Convertregular(float *, float *);
...
/*
	...
	*/
	...
	for (int i = 0; i < 68; i++)
	{
		...
		{
			float x = X_READ[i][j] + TransXYZ[0];
			float y = Y_READ[i][j] + TransXYZ[1];

			//ConvertHyperbolic(&x, &y);
			if (T_intersect_elim) {
				if (j == 0) {
					xs = x;
					ys = y;
					...
					ye = y;
					for (int i = 0; i < 100; i++) {
						float x_temp; float y_temp;
						x_temp = (1. - i * 0.01) * xs + i * 0.01 * xe;
						y_temp = (1. - i * 0.01) * ys + i * 0.01 * ye;
						if (Check_cartisan) {
							ConvertCartisan(&x_temp, &y_temp);
							glVertex2f(x_temp, y_temp);
						}
						else if (Check_polar) {
							ConvertPolar(&x_temp, &y_temp);
							glVertex2f(x_temp, y_temp);
						}
						else {
							Convertregular(&x_temp, &y_temp);
							glVertex2f(x_temp, y_temp);
						}
					}
					xs = xe;
...
InitGlui()
{
	...
	Glui->add_checkbox("T-Intersection Elimination", &T_intersect_elim);
	Glui->add_checkbox("Polar", &Check_polar);
	Glui->add_checkbox("Cartesian", &Check_cartisan);
...
InitGraphics()
{
	...
	for (int i = 0; i < 68; i++)
	{
		...
		for (int j = 0; j < Npts[i]; j++)
		{
			fscanf(fp, "%f %f", &X_READ[i][j], &Y_READ[i][j]);
		}
	}
	fclose(fp);
...
Reset()
{
	...
	Grayscale = GLUITRUE;
	PointsOn = GLUITRUE;
	T_intersect_elim = GLUIFALSE;
	Check_polar = GLUIFALSE;
	Check_cartisan = GLUIFALSE;
	Scale = 1.0;
	Scale2 = 0.0;
...
void ConvertHyperbolic(float *x, float *y) {
	Tx += Xmouse - LastX;
	Ty += Ymouse - LastY;
	LastX = Xmouse;
	LastY = Ymouse;
...
void ConvertPolar(float *x, float *y) {
	float x1, y1;
	x1 = *x;
	y1 = *y;
	float r = sqrt(x1*x1 + y1 * y1);
	float polar_x = x1 / (r + SLowHigh[0]);
	float polar_y = y1 / (r + SLowHigh[0]);
	*x = polar_x;
	*y = polar_y;
}

void ConvertCartisan(float *x, float *y) {
	float x1, y1;
	x1 = *x;
	y1 = *y;
	float cartisan_x = x1 / sqrt(x1*x1 + SLowHigh[0] * SLowHigh[0]);
	float cartisan_y = y1 / sqrt(y1*y1 + SLowHigh[0] * SLowHigh[0]);
	*x = cartisan_x;
	*y = cartisan_y;
}

void Convertregular(float *x, float *y) {
	float x1, y1;
	x1 = *x;
	y1 = *y;
	float x1_temp = x1 / SLowHigh[0];
	float y1_temp = y1 / SLowHigh[0];
	*x = x1_temp;
	*y = y1_temp;
}
