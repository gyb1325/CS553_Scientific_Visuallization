...
const char *WINDOWTITLE = { "Project02 --Yongbin Gu" };
	...
	for (int dx = 0; dx < NX; dx++) {
		for (int dy = 0; dy < NY; dy++) {
			for (int dz = 0; dz < NZ; dz++) {
				if (Nodes[dx][dy][dz].x < XLowHigh[0] || Nodes[dx][dy][dz].x > XLowHigh[1])
					continue;
				if (Nodes[dx][dy][dz].y < YLowHigh[0] || Nodes[dx][dy][dz].y > YLowHigh[1])
					continue;
				if (Nodes[dx][dy][dz].z < ZLowHigh[0] || Nodes[dx][dy][dz].z > ZLowHigh[1])
					continue;
				if (Nodes[dx][dy][dz].t < SLowHigh[0] || Nodes[dx][dy][dz].t > SLowHigh[1])
					continue;
				if (Nodes[dx][dy][dz].rad < RLowHigh[0] || Nodes[dx][dy][dz].rad > RLowHigh[1])
					continue;
				if (Nodes[dx][dy][dz].grad < GLowHigh[0] || Nodes[dx][dy][dz].grad > GLowHigh[1])
					continue;
				float t = Nodes[dx][dy][dz].t;
				float hsv[3], rgb[3];
				hsv[0] = 240. - 240. * (t - SMIN) / (SMAX - SMIN);
				hsv[1] = 1.0;
				hsv[2] = 1.0;
				HsvRgb(hsv, rgb);
				glColor3fv(rgb);
				glVertex3f(Nodes[dx][dy][dz].x, Nodes[dx][dy][dz].y, Nodes[dx][dy][dz].z);
			}
		}
	}
...
InitGraphics( )
{
	...
	for (int dx = 0; dx < NX; dx++) {
		for (int dy = 0; dy < NY; dy++) {
			for (int dz = 0; dz < NZ; dz++) {
				float tempx = -1.0 + 2.0 / (NX - 1) * dx;
				float tempy = -1.0 + 2.0 / (NY - 1) * dy;
				float tempz = -1.0 + 2.0 / (NZ - 1) * dz;
				float temp_t = Temperature(tempx, tempy, tempz);
				Nodes[dx][dy][dz].x = tempx;
				Nodes[dx][dy][dz].y = tempy;
				Nodes[dx][dy][dz].z = tempz;
				Nodes[dx][dy][dz].rad = sqrt(tempx*tempx + tempy * tempy + tempz * tempz);
				if (temp_t < SMIN) {
					temp_t = SMIN;
				}
				else if (temp_t > SMAX) {
				...
				Nodes[dx][dy][dz].t = temp_t;
				if (dx > 0 && dx < NX - 1)
				{
					Nodes[dx][dy][dz].dTdx = (Nodes[dx + 1][dy][dz].t - Nodes[dx - 1][dy][dz].t) / (Nodes[dx + 1][dy][dz].x - Nodes[dx - 1][dy][dz].x);
				}
				else if (dx == 0)
				{
					Nodes[dx][dy][dz].dTdx = (Nodes[dx + 1][dy][dz].t - Nodes[dx][dy][dz].t) / (Nodes[dx + 1][dy][dz].x - Nodes[dx][dy][dz].x);
				}
				else {
					Nodes[dx][dy][dz].dTdx = (Nodes[dx][dy][dz].t - Nodes[dx - 1][dy][dz].t) / (Nodes[dx][dy][dz].x  - Nodes[dx - 1][dy][dz].x);
				}

				if (dy > 0 && dy < NY - 1)
				{
					Nodes[dx][dy][dz].dTdy = (Nodes[dx][dy + 1][dz].t - Nodes[dx][dy - 1][dz].t) / (Nodes[dx][dy + 1][dz].y - Nodes[dx][dy - 1][dz].y);
				}
				else if (dy == 0)
				{
					Nodes[dx][dy][dz].dTdy = (Nodes[dx][dy + 1][dz].t - Nodes[dx][dy][dz].t) / (Nodes[dx][dy + 1][dz].y - Nodes[dx][dy][dz].y);
				}
				else {
					Nodes[dx][dy][dz].dTdy = (Nodes[dx][dy][dz].t -Nodes[dx][dy - 1][dz].t) / (Nodes[dx][dy][dz].y -Nodes[dx][dy - 1][dz].y);
				}

				if (dz > 0 && dz < NZ - 1)
				{
					Nodes[dx][dy][dz].dTdz = (Nodes[dx][dy][dz + 1].t - Nodes[dx][dy][dz - 1].t) / (Nodes[dx][dy][dz+1].z - Nodes[dx][dy][dz - 1].z);
				}
				else if (dz == 0)
				{
					Nodes[dx][dy][dz].dTdz = (Nodes[dx][dy][dz + 1].t - Nodes[dx][dy][dz].t) / (Nodes[dx][dy][dz+1].z - Nodes[dx][dy][dz].z);
				}
				else {
					Nodes[dx][dy][dz].dTdz = (Nodes[dx][dy][dz].t -Nodes[dx][dy][dz - 1].t) / (Nodes[dx][dy][dz].z -Nodes[dx][dy][dz - 1].z);
				}
				Nodes[dx][dy][dz].grad = sqrt(SQR(Nodes[dx][dy][dz].dTdx) + SQR(Nodes[dx][dy][dz].dTdy) + SQR(Nodes[dx][dy][dz].dTdy));
				if (max_grad < Nodes[dx][dy][dz].grad) {
					max_grad = Nodes[dx][dy][dz].grad;
				}
			}
		}
	}
	fprintf(stdout, "MAx is %5.2lf", max_grad);
