...
const char *WINDOWTITLE = { "Project01 -- Yongbin Gu" };
...
const float	TEMP_MIN = { 0.f };
const float TEMP_MAX = { 100.f };

const float GRAY_MIN = { 0.20f };
const float GRAY_MAX = { 1.0f };
	...
	for (int dx = 0; dx < NX; dx++) {
		for (int dy = 0; dy < NY; dy++) {
			for (int dz = 0; dz < NZ; dz++) {
				...
				float gray = GRAY_MIN + (GRAY_MAX - GRAY_MIN) * (t - TEMP_MIN) / (TEMP_MAX - TEMP_MIN);
